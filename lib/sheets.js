...
export async function getCachedTeams() {
    try {
        ...
        if (!sheet) {
            console.log('Veritabanı sheet not found');
            return [];
        }

        await sheet.loadHeaderRow();
        const rows = await sheet.getRows();
        console.log(`Found ${rows.length} teams in cache`);

        return rows.map(row => {
            const logo = row.get('Logo URL');
            // URL temizleme: \/ karakterlerini / ile değiştir
            const cleanLogo = logo ? String(logo).replace(/\\\//g, '/') : null;
